...
static void cio_bytecode_internal_emit_cleanup_offsets(uint32_t** offsets) {
    if(!*offsets) return;
...
static void cio_bytecode_internal_emit_cleanup_code(unsigned char** code) {
    if(!*code) return;
...
gen_error_t* cio_bytecode_emit(const cio_program_t* const restrict program, unsigned char** const restrict out_bytecode, size_t* const restrict out_bytecode_length, const char* const restrict source, GEN_UNUSED const size_t source_length, const char* const restrict source_file, GEN_UNUSED const size_t source_file_length) {
	...
	GEN_CLEANUP_FUNCTION(cio_bytecode_internal_emit_cleanup_offsets) uint32_t* offsets = NULL;
	...
	size_t code_size = 0;
	GEN_CLEANUP_FUNCTION(cio_bytecode_internal_emit_cleanup_code) unsigned char* code = NULL;
	// Codegen
	{
	...
	GEN_CLEANUP_FUNCTION(cio_bytecode_internal_emit_cleanup_code) unsigned char* header = NULL;
	...
	*out_bytecode_length = header_size + code_size;
	...
	if(error) return error;
    GEN_CLEANUP_FUNCTION(cio_bytecode_internal_emit_cleanup_code) unsigned char* bytecode_cleanup = *out_bytecode;
